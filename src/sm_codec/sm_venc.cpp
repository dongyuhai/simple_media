...
typedef struct sm_venc_param {
    ...
    sm_status_t(*sm_venc_encode)(void* handle, sm_picture_info_t *p_frame, uint32_t force_idr, int64_t pts);
...
sm_status_t sm_venc_encode(sm_venc_handle_t handle, sm_picture_info_t *p_frame, uint32_t force_idr, int64_t pts)
{
